...
func configureProductAdvancedSettings(advanced_setting map[string]interface{}, productConfig *ProductConfig) {
	...
	if advanced_setting["enforce_test_failure_if_runs_longer_than"] != nil {
		enforce_test_failure_if_runs_longer_than := advanced_setting["enforce_test_failure_if_runs_longer_than"].(int)
		productConfig.MaxStepRuntimeSecOverride = enforce_test_failure_if_runs_longer_than

	}

	if advanced_setting["verify_test_on_failure"] != nil {
		verify_test_on_failure := advanced_setting["verify_test_on_failure"].(bool)
		productConfig.VerifytestOnFailure = verify_test_on_failure
	}

	if advanced_setting["viewport_height"] != nil {
		viewport_height := advanced_setting["viewport_height"].(int)
		productConfig.ViewportHeight = viewport_height
	}
	if advanced_setting["viewport_width"] != nil {
		viewport_width := advanced_setting["viewport_width"].(int)
		productConfig.ViewportWidth = viewport_width

	}
	wait_for_no_activity := advanced_setting["wait_for_no_activity"].(int)
	if advanced_setting["stop_test_on_document_complete"].(bool) {
		productConfig.WaitForNoActivityOnDocComplete = &wait_for_no_activity
	}
	if advanced_setting["bandwidth_throttling"] != nil {
		bandwidth_throttling := advanced_setting["bandwidth_throttling"].(string)
		bandwidth_throttling_id, bandwidth_throttling_name := getBandwidthThrottlingTypeId(bandwidth_throttling)
		productConfig.BandwidthThrottling.Id = bandwidth_throttling_id
		productConfig.BandwidthThrottling.Name = bandwidth_throttling_name
	}
	if advanced_setting["edns_subnet"] != nil {
		edns_subnet := advanced_setting["edns_subnet"].(string)
		productConfig.EdnsSubnet = edns_subnet
	}
	if advanced_setting["additional_monitor"] != nil {
		additional_monitor := advanced_setting["additional_monitor"].(string)
		additional_monitor_id, additional_monitor_name := getAdditionalMonitorTypeId(additional_monitor)
		productConfig.AdditionalMonitorType.Id = additional_monitor_id
		productConfig.AdditionalMonitorType.Name = additional_monitor_name
	}
	if advanced_setting["ping_count"] != nil {
		ping_count := advanced_setting["ping_count"].(int)
		productConfig.TraceroutePingCount = ping_count

	}
	if advanced_setting["failure_hop_count"] != nil {
		failure_hop_count := advanced_setting["failure_hop_count"].(int)
		productConfig.TracerouteFailureHopCount = failure_hop_count
	}

	productConfig.AdvancedSettingType = 1
	productConfig.AppliedTestFlags = applied_test_flag_ids

}

func configureFolderAdvancedSettings(advanced_setting map[string]interface{}, folderConfig *FolderConfig) {
	var applied_test_flag_ids []int
	...
	if advanced_setting["enforce_test_failure_if_runs_longer_than"] != nil {
		enforce_test_failure_if_runs_longer_than := advanced_setting["enforce_test_failure_if_runs_longer_than"].(int)
		folderConfig.MaxStepRuntimeSecOverride = enforce_test_failure_if_runs_longer_than

	}

	if advanced_setting["verify_test_on_failure"] != nil {
		verify_test_on_failure := advanced_setting["verify_test_on_failure"].(bool)
		folderConfig.VerifytestOnFailure = verify_test_on_failure
	}

	if advanced_setting["viewport_height"] != nil {
		viewport_height := advanced_setting["viewport_height"].(int)
		folderConfig.ViewportHeight = viewport_height
	}
	if advanced_setting["viewport_width"] != nil {
		viewport_width := advanced_setting["viewport_width"].(int)
		folderConfig.ViewportWidth = viewport_width

	}
	wait_for_no_activity := advanced_setting["wait_for_no_activity"].(int)
	if advanced_setting["stop_test_on_document_complete"].(bool) {
		folderConfig.WaitForNoActivityOnDocComplete = &wait_for_no_activity
	}
	if advanced_setting["bandwidth_throttling"] != nil {
		bandwidth_throttling := advanced_setting["bandwidth_throttling"].(string)
		bandwidth_throttling_id, bandwidth_throttling_name := getBandwidthThrottlingTypeId(bandwidth_throttling)
		folderConfig.BandwidthThrottling.Id = bandwidth_throttling_id
		folderConfig.BandwidthThrottling.Name = bandwidth_throttling_name
	}
	if advanced_setting["edns_subnet"] != nil {
		edns_subnet := advanced_setting["edns_subnet"].(string)
		folderConfig.EdnsSubnet = edns_subnet
	}
	if advanced_setting["additional_monitor"] != nil {
		additional_monitor := advanced_setting["additional_monitor"].(string)
		additional_monitor_id, additional_monitor_name := getAdditionalMonitorTypeId(additional_monitor)
		folderConfig.AdditionalMonitorType.Id = additional_monitor_id
		folderConfig.AdditionalMonitorType.Name = additional_monitor_name
	}
	if advanced_setting["ping_count"] != nil {
		ping_count := advanced_setting["ping_count"].(int)
		folderConfig.TraceroutePingCount = ping_count

	}
	if advanced_setting["failure_hop_count"] != nil {
		failure_hop_count := advanced_setting["failure_hop_count"].(int)
		folderConfig.TracerouteFailureHopCount = failure_hop_count
	}

	folderConfig.AdvancedSettingType = 1
	folderConfig.AppliedTestFlags = applied_test_flag_ids
...
func configureProductRequestSettings(request_setting map[string]interface{}, productConfig *ProductConfig) error {
	...
	for i := range authentication_list {
		...
		productConfig.AuthenticationType.Id = authentication_type_id
		productConfig.AuthenticationType.Name = authentication_type_name
		...
		productConfig.AuthenticationPasswordIds = password_ids
	}
	...
	productConfig.AuthenticationTokenIds = token_ids
	...
	if request_setting["library_certificate_ids"] != nil {
		...
		productConfig.AuthenticationCertificateIds = certificate_ids
	}
	...
	productConfig.RequestSettingType = 1

	return nil
}

func configureFolderRequestSettings(request_setting map[string]interface{}, folderConfig *FolderConfig) error {
	authentication_list := request_setting["authentication"].(*schema.Set).List()
	for i := range authentication_list {
		authentication := authentication_list[i].(map[string]interface{})
		authentication_type := authentication["authentication_type"].(string)
		...
		folderConfig.AuthenticationType.Id = authentication_type_id
		folderConfig.AuthenticationType.Name = authentication_type_name
		...
		folderConfig.AuthenticationPasswordIds = password_ids
	}
	...
	folderConfig.AuthenticationTokenIds = token_ids
	...
	if request_setting["library_certificate_ids"] != nil {
		...
		folderConfig.AuthenticationCertificateIds = certificate_ids
	}
	...
	folderConfig.RequestSettingType = 1
...
func configureProductInsightSettings(insight_setting map[string]interface{}, productConfig *ProductConfig) {
	...
	productConfig.InsightSettingType = 1
	productConfig.TracepointIds = tracepoint_ids
	productConfig.IndicatorIds = indicator_ids
}

func configureFolderInsightSettings(insight_setting map[string]interface{}, folderConfig *FolderConfig) {
	...
	folderConfig.InsightSettingType = 1
	folderConfig.TracepointIds = tracepoint_ids
	folderConfig.IndicatorIds = indicator_ids
}
...
func configureProductScheduleSettings(schedule_setting map[string]interface{}, productConfig *ProductConfig) error {
	frequency := schedule_setting["frequency"].(string)
	frequency_id, frequency_name := getFrequencyId(frequency)
	if frequency_id == -1 {
		return errors.New("invalid test scheduling frequency string provided. acceptable values are 1 minute, 2 minutes, 4 minutes, 5 minutes, 10 minutes, 15 minutes, 20 minutes, 30 minutes, 60 minutes, 2 hours, 3 hours, 4 hours, 6 hours, 8 hours, 12 hours, 24 hours")
	...
	productConfig.ScheduleSettingType = 1
	productConfig.ScheduleRunScheduleId = run_schedule_id
	productConfig.ScheduleMaintenanceScheduleId = maintenance_schedule_id
	productConfig.TestFrequency.Id = frequency_id
	productConfig.TestFrequency.Name = frequency_name
	productConfig.NodeDistribution.Id = node_distribution_id
	productConfig.NodeDistribution.Name = node_distribution_name
	productConfig.NodeIds = node_ids
	productConfig.NodeGroupIds = nodeGroupIds

	if no_of_subset_nodes > 0 {
		productConfig.NoOfSubsetNodes = no_of_subset_nodes
	}

	return nil
}

func configureFolderScheduleSettings(schedule_setting map[string]interface{}, folderConfig *FolderConfig) error {
	frequency := schedule_setting["frequency"].(string)
	frequency_id, frequency_name := getFrequencyId(frequency)
	if frequency_id == -1 {
		return errors.New("invalid test scheduling frequency string provided. acceptable values are 1 minute, 2 minutes, 4 minutes, 5 minutes, 10 minutes, 15 minutes, 20 minutes, 30 minutes, 60 minutes, 2 hours, 3 hours, 4 hours, 6 hours, 8 hours, 12 hours, 24 hours")
	...
	folderConfig.ScheduleSettingType = 1
	folderConfig.ScheduleRunScheduleId = run_schedule_id
	folderConfig.ScheduleMaintenanceScheduleId = maintenance_schedule_id
	folderConfig.TestFrequency.Id = frequency_id
	folderConfig.TestFrequency.Name = frequency_name
	folderConfig.NodeDistribution.Id = node_distribution_id
	folderConfig.NodeDistribution.Name = node_distribution_name
	folderConfig.NodeIds = node_ids
	folderConfig.NodeGroupIds = nodeGroupIds

	if no_of_subset_nodes > 0 {
		folderConfig.NoOfSubsetNodes = no_of_subset_nodes
	}
...
func setAlertSettings(testTypeId int, alert_setting map[string]interface{}, testConfig *TestConfig) error {
	...
	testConfig.AlertSettingType = IdName{Id: alert_setting_type_id, Name: alert_setting_type_name}
...
func configureProductAlertSettings(alert_setting map[string]interface{}, productConfig *ProductConfig) error {
	...
	for i := range alert_rule_list {
		...
		productConfig.AlertRuleConfigs = append(productConfig.AlertRuleConfigs, AlertRuleConfig{AlertNodeThresholdType: IdName{Id: node_threshold_type_id, Name: node_threshold_type_name}, AlertThresholdNumOfRuns: threshold_number_of_runs, AlertConsecutiveNumOfRuns: consecutive_number_of_runs, AlertThresholdPercentOfRuns: threshold_percentage_of_runs,
			AlertThresholdNumOfFailingNodes: number_of_failing_nodes, TriggerType: IdName{Id: trigger_type_id, Name: trigger_type_name},
			OperationType:              IdName{Id: operation_type_id, Name: operation_type_name},
			StatisticalType:            IdName{Id: statistical_type_id, Name: statistical_type_name},
			TrailingHistoricalInterval: IdName{Id: historical_interval_id, Name: historical_interval_name},
	...
	productConfig.AlertSubject = subject

	return nil
}

func configureFolderAlertSettings(alert_setting map[string]interface{}, folderConfig *FolderConfig) error {
	var alert_setting_type string
	...
	for i := range alert_rule_list {
		...
		folderConfig.AlertRuleConfigs = append(folderConfig.AlertRuleConfigs, AlertRuleConfig{AlertNodeThresholdType: IdName{Id: node_threshold_type_id, Name: node_threshold_type_name}, AlertThresholdNumOfRuns: threshold_number_of_runs, AlertConsecutiveNumOfRuns: consecutive_number_of_runs, AlertThresholdPercentOfRuns: threshold_percentage_of_runs,
			AlertThresholdNumOfFailingNodes: number_of_failing_nodes, TriggerType: IdName{Id: trigger_type_id, Name: trigger_type_name},
			OperationType:              IdName{Id: operation_type_id, Name: operation_type_name},
			StatisticalType:            IdName{Id: statistical_type_id, Name: statistical_type_name},
			TrailingHistoricalInterval: IdName{Id: historical_interval_id, Name: historical_interval_name},
	...
	folderConfig.AlertSettingType = IdName{Id: alert_setting_type_id, Name: alert_setting_type_name}
	folderConfig.AlertWebhookIds = all_alert_webhook_ids
	folderConfig.AlertRecipientEmails = all_email_ids
	folderConfig.AlertContactGroups = all_contact_groups
	folderConfig.AlertSubject = subject
