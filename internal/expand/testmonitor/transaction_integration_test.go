...
func TestExpandTransactionTestConfigFromPlan(t *testing.T) {
	test := []struct {
		monitor   string
		monitorID int
	}{
	...
	for _, tt := range test {
		...
		plan := testmonitor.TransactionTestResourceModel{
			...
			TestScriptResourceModel: resource.TestScriptResourceModel{
				...
				ScriptType: types.StringValue("selenium"),
			},
		...
		// Verify that the Transaction monitor fields were set correctly
		...
		testutil.AssertEqual(t, "test type ID", config.TestType.ID, 1)
		testutil.AssertEqual(t, "test type name", config.TestType.Name, "transaction")
		...
		testutil.AssertEqual(t, fields.TestScriptType, config.Script.TransactionScriptType.Name, "selenium")
		testutil.AssertEqual(t, fields.TestScriptType+"ID", config.Script.TransactionScriptType.ID, 1)
		testutil.AssertEqual(t, fields.TestScript+"Type", config.Script.TransactionScriptType.Name, "selenium")
...
func TestExpandTransactionTestConfigFromPlanChromeVersion(t *testing.T) {
	test := []struct {
...
func TestExpandTransactionTestConfigFromPlanSimulateField(t *testing.T) {
	test := []struct {
		userAgent   string
		userAgentID int
	}{
