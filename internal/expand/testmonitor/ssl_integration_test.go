...
func TestExpandSSLTestConfigFromPlan(t *testing.T) {
	...
	plan := testmonitor.SSLTestResourceModel{
		...
		TestLocation: types.StringValue("example.com"),
		...
		SSLSettingsModel: resource.SSLSettingsModel{
			EnforceCertificateKeyPinning: types.BoolValue(true),
			EnforceCertificatePinning:    types.BoolValue(false),
			FileData:                     types.StringValue("filedata"),
	...
	testutil.AssertEqual(t, fields.TestName, config.TestName, "SSL Monitor")

	// Verify optional fields were set correctly
	testutil.AssertEqual(t, fields.FolderID, config.FolderID, 789)
	testutil.AssertEqual(t, fields.TestDescription, config.TestDescription, "SSL description")
	...
	// Verify that the SSL monitor fields were set correctly
	testutil.AssertEqual(t, fields.Monitor+"ID", config.Monitor.ID, 31)
	testutil.AssertEqual(t, fields.Monitor+"Name", config.Monitor.Name, "ssl")
	testutil.AssertEqual(t, "test type ID", config.TestType.ID, 18)
	testutil.AssertEqual(t, "test type name", config.TestType.Name, "ssl")

	// For an SSL test, the URL should not be set.
	...
	testutil.AssertEqual(t, "EnforceCertificateKeyPinning", config.EnforceCertificateKeyPinning, true)
	testutil.AssertEqual(t, "EnforceCertificatePinning", config.EnforceCertificatePinning, false)
	testutil.AssertEqual(t, "FileData", config.FileData, "filedata")
	testutil.AssertEqual(t, "Passphrase", config.Passphrase, "passphrase")
}
