...
func TestExpandTracerouteTestConfigFromPlan(t *testing.T) {
	test := []struct {
		monitor   string
		monitorID int
	}{
	...
	for _, tt := range test {
		...
		testutil.AssertEqual(t, fields.TestName, config.TestName, "Traceroute Monitor")

		// Verify optional fields were set correctly
		testutil.AssertEqual(t, fields.FolderID, config.FolderID, 789)
		testutil.AssertEqual(t, fields.TestDescription, config.TestDescription, "Traceroute description")
		...
		// Verify that the Traceroute monitor fields were set correctly
		...
		testutil.AssertEqual(t, "test type ID", config.TestType.ID, 12)
		testutil.AssertEqual(t, "test type name", config.TestType.Name, "traceroute")

		// For an Traceroute test, the URL should not be set.
