...
func TestExpandAPITestConfigFromPlan(t *testing.T) {
	script := `// Step - 1 \nname("step")\nname("step 1")\nopen("https://www.amazon.com")`

	// Create a fully populated plan
	plan := testmonitor.APITestResourceModel{
		...
		TestScriptResourceModel: resource.TestScriptResourceModel{
			...
			ScriptType: types.StringValue("javascript"),
		},
	...
	// Verify that the API monitor fields were set correctly
	testutil.AssertEqual(t, fields.Monitor+"ID", config.Monitor.ID, 25)
	testutil.AssertEqual(t, fields.Monitor+"Name", config.Monitor.Name, "api")
	testutil.AssertEqual(t, "test type ID", config.TestType.ID, 9)
	testutil.AssertEqual(t, "test type name", config.TestType.Name, "api")

	// For an API test, the URL should not be set.
	...
	testutil.AssertEqual(t, fields.TestScriptType, config.Script.TransactionScriptType.Name, "javascript")
	testutil.AssertEqual(t, fields.TestScriptType+"ID", config.Script.TransactionScriptType.ID, 2)
	testutil.AssertEqual(t, fields.TestScript+"Type", config.Script.TransactionScriptType.Name, "javascript")

	// Monitor and TestType should be replicated into the Script block.
	testutil.AssertEqual(t, fields.Monitor+"ID", config.Script.Monitor.ID, 25)
	testutil.AssertEqual(t, fields.Monitor+"Name", config.Script.Monitor.Name, "api")
	testutil.AssertEqual(t, "test type ID", config.Script.TestType.ID, 9)
	testutil.AssertEqual(t, "test type name", config.Script.TestType.Name, "api")
