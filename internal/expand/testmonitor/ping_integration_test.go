...
func TestExpandPingTestConfigFromPlan(t *testing.T) {
	test := []struct {
		monitor   string
		monitorID int
	}{
	...
	for _, tt := range test {
		...
		plan := testmonitor.PingTestResourceModel{
			BaseTestResourceModel: resource.BaseTestResourceModel{
				...
				Description:           types.StringValue("Ping description"),
		...
		testutil.AssertEqual(t, fields.TestName, config.TestName, "Ping Monitor")

		// Verify optional fields were set correctly
		testutil.AssertEqual(t, fields.FolderID, config.FolderID, 789)
		testutil.AssertEqual(t, fields.TestDescription, config.TestDescription, "Ping description")
		...
		// Verify that the Ping monitor fields were set correctly
		...
		testutil.AssertEqual(t, "test type ID", config.TestType.ID, 6)
		testutil.AssertEqual(t, "test type name", config.TestType.Name, "ping")

		// For an Ping test, the URL should not be set.
