...
func TestExpandPuppeteerTestConfigFromPlan(t *testing.T) {
	script := `// Step - 1 \nname("step")\nname("step 1")\nopen("https://www.amazon.com")`

	// Create a fully populated plan
	plan := testmonitor.PuppeteerTestResourceModel{
		...
		TestScriptResourceModel: resource.TestScriptResourceModel{
			...
			ScriptType: types.StringValue("puppeteer"),
		},
	...
	// Verify that the Puppeteer monitor fields were set correctly
	testutil.AssertEqual(t, fields.Monitor+"ID", config.Monitor.ID, 18)
	testutil.AssertEqual(t, fields.Monitor+"Name", config.Monitor.Name, "chrome")
	testutil.AssertEqual(t, "test type ID", config.TestType.ID, 26)
	testutil.AssertEqual(t, "test type name", config.TestType.Name, "puppeteer")

	// For an Puppeteer test, the URL should not be set.
	...
	testutil.AssertEqual(t, fields.TestScriptType, config.Script.TransactionScriptType.Name, "puppeteer")
	testutil.AssertEqual(t, fields.TestScriptType+"ID", config.Script.TransactionScriptType.ID, 4)
	testutil.AssertEqual(t, fields.TestScript+"Type", config.Script.TransactionScriptType.Name, "puppeteer")

	// Monitor and TestType should be replicated into the Script block.
	testutil.AssertEqual(t, fields.Monitor+"ID_ScriptBlock", config.Script.Monitor.ID, 18)
	testutil.AssertEqual(t, fields.Monitor+"Name_ScriptBlock", config.Script.Monitor.Name, "chrome")
	testutil.AssertEqual(t, "ScriptBlock testtype.ID", config.Script.TestType.ID, 26)
	testutil.AssertEqual(t, "ScriptBlock testtype.name", config.Script.TestType.Name, "puppeteer")
