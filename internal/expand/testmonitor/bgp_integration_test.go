...
func TestExpandBGPTestConfigFromPlan(t *testing.T) {
	// Create a fully populated plan
	plan := testmonitor.BGPTestResourceModel{
		Prefix: types.StringValue("192.168.0.0/24"),
	...
	testutil.AssertEqual(t, fields.Prefix, config.TestURL, "192.168.0.0/24")
	...
	// Verify that the BGP monitor fields were set correctly
	testutil.AssertEqual(t, fields.Monitor+"ID", config.Monitor.ID, 34)
	testutil.AssertEqual(t, fields.Monitor+"Name", config.Monitor.Name, "bgp")
	testutil.AssertEqual(t, "test type ID", config.TestType.ID, 20)
	testutil.AssertEqual(t, "test type name", config.TestType.Name, "bgp")
